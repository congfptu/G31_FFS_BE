...
public class PostController {
    ...
    public ResponseEntity<?> getAllPostSearchByDesAndStatusPaging(@RequestHeader(name = "Authorization") String token,
        ...
        int totalPage = postRepository.getRequestPostByStatusAndDescription(keyword, status,p).getTotalPages();

        if (totalPage >= pageIndex - 1&& status.equals("-1")) {
        ...
        else if (totalPage >= pageIndex - 1){
            PostDTOResponse fas = postService.getAllPostByNameAndStatusPaging(pageIndex, pageSize, keyword,status, null);
            return new ResponseEntity<>(fas, HttpStatus.OK);
            }
         else {
